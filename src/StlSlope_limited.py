...
class StlSlope():
    ...
    def slope_strategy (self)   :
        ...
        for i in range(self.period-1,self.residual.shape[0]-1):  
            ...
            # sell for save
            if self.df['close'][i]/mem <0.92 and position==1 and a<=0:
                lis.append(-1)
                position=0
                continue
            # buy
            if self.residual.values[i]<middle*0.8 and a>0.1 and position==0:
                lis.append(1)
                position=1 
                mem = self.df['close'][i]     
                continue 
            # sell
            elif self.residual.values[i]>middle and a>=-0.3 and position==1:
                lis.append(-1)
                position=0
                continue
            else:
